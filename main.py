...
@requires_auth('dev')
def get_product_by_id(jwt, product_id):  
  try:
...
@requires_auth('dev')
def get_customer_by_id(jwt, customer_id):
  try:
...
@requires_auth('dev')
def get_cart_items(jwt, customer_id):
  try:
...
def update_category(jwt, category_id) :## admin role
  try:
    json_body = request.get_json()
